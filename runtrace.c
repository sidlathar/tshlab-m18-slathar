...
void sigalrm_handler(int sig) {
    ...
    if (sigfillset(&mask_all) < 0) {
        sio_error("sigfillset error in sigalrm_handler\n");
    }
    if (sigprocmask(SIG_BLOCK, &mask_all, &prev_all) < 0) {
        sio_error("sigprocmask error in sigalrm_handler\n");
    }
...
void graceful_cleanup_sig_handler(int sig) {
    // block signal
    sigset_t mask_all, prev_all;
    sio_puts("Cleaning up and exiting\n");

    if (sigfillset(&mask_all) < 0) {
        sio_error("sigfillset error in signal_handler\n");
    }
    if (sigprocmask(SIG_BLOCK, &mask_all, &prev_all) < 0) {
        sio_error("sigprocmask error in signal_handler\n");
    }
    ...
    // reraise signal
    if (raise(sig) < 0) {
        sio_error("signal re-raise error.");
    }
    // unblock signals
...
int main(int argc, char **argv) {
    ...
    // block signals
    if (sigfillset(&mask_all) < 0) {
        sio_error("sigfillset error in main\n");
    }
    if (sigprocmask(SIG_BLOCK, &mask_all, &prev_all) < 0) {
        sio_error("sigprocmask error in main\n");
    }
    ...
    if (mkdir(RUNTRACE_TMP_FOLDER, 0700) != 0) {
        if(errno == EEXIST) {
            fprintf(stderr, "Runtrace temporary directory (%s) exists.\n"
                    "runtrace will destroy this directory content, refusing to continue.\n"
                    "please move of remove this directory.\n", RUNTRACE_TMP_FOLDER);
            sio_error("tmp directory exists\n");
        } else {
           sio_error("failed to create tmp directory");
        }
    }
    ...
    if ((child_pid  = fork()) == 0) {
        ...
        // unblock signals.
        if (sigprocmask(SIG_SETMASK, &prev_all, NULL) < 0) {
            sio_error("sigprocmask error in fork\n");
        }
        pid_t session_id;
    ...
    while (fgets_result) {
         ...
         * sends fg/bg $(pgrep -s session_id name) to the shell */
            ...
            sprintf(command, "/usr/bin/pgrep -s %d %s", child_pid, name);
            ...
            sprintf(command, "%s %s", builtin, name);
    ...
    // block signals
    if (sigfillset(&mask_all) < 0) {
        sio_error("sigfillset error in main cleanup\n");
    }
    if (sigprocmask(SIG_BLOCK, &mask_all, &prev_all) < 0) {
        sio_error("sigprocmask error in main cleanup\n");
    }
    clean(prev_all);
    //unblock signals
    if (sigprocmask(SIG_SETMASK, &prev_all, NULL) < 0) {
...
void atexit_clean() {
    if (cleanup_needed) {
        ...
        if (sigprocmask(SIG_SETMASK, NULL, &prev_all) < 0) {
            sio_error("sigprocmask error in atexit_clean\n");
        }
        clean(prev_all);
    }
}
